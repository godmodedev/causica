...
def apply_image_preprocessing(obs: dict, function: Callable) -> dict:
    """Applies arbitary image preprocessing function to all images in the observation"""
    for key, value in obs.items():
        if key.startswith("image_") and value.dtype == tf.uint8:
            obs[key] = function(value)
    return obs
