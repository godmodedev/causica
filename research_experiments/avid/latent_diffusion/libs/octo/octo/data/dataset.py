...
class DLatasetWithFixedSeed(dl.DLataset):
    @staticmethod
    def from_rlds_with_seed(
        seed: int,
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    dataset = dataset.traj_map(traj_transforms.add_pad_mask_dict, num_parallel_calls, deterministic=deterministic)
...
def apply_frame_transforms(
    ...
    *,
    train: bool,
    preprocess_image_fn: Callable = None,
    ...
    Args:
        ...
        preprocess_image_fn (Callable): Arbitrary preprocessing function to apply to all images.
        dataset (dl.DLataset): The dataset to transform.
    ...
    if train:
        ...
        dataset = dataset.frame_map(aug, num_parallel_calls, deterministic=deterministic)

    if preprocess_image_fn is not None:
        dataset = dataset.frame_map(
            partial(
                apply_obs_transform, partial(obs_transforms.apply_image_preprocessing, function=preprocess_image_fn)
            ),
            num_parallel_calls,
            deterministic=deterministic,
        )
...
def make_dataset_from_rlds(
    ...
    data_dir: str,
    seed: int = 0,
    save_statistics_dir: str | None = None,
    deterministic: bool = False,
    *,
    train: bool,
    ...
    shuffle: bool = True,
    image_obs_keys: Mapping[str, str | None] = {},
    depth_obs_keys: Mapping[str, str | None] = {},
    state_obs_keys: Sequence[str | None] = (),
    language_key: str | None = None,
    action_proprio_normalization_type: NormalizationType = NormalizationType.NORMAL,
    dataset_statistics: dict | str | None = None,
    absolute_action_mask: Sequence[bool] | None = None,
    action_normalization_mask: Sequence[bool] | None = None,
    norm_skip_keys: Sequence[str] | None = None,
    ...
    if isinstance(dataset_statistics, str):
        ...
        dataset_statistics = get_dataset_statistics(
            ...
            save_dir=save_statistics_dir if save_statistics_dir is not None else builder.data_dir,
        )
    ...
    if train:
        ...
        dataset = DLatasetWithFixedSeed.from_rlds_with_seed(
            seed=seed, builder=builder, split=split, shuffle=shuffle, num_parallel_reads=num_parallel_reads
        )

    for filter_fcn_spec in filter_functions:
        dataset = dataset.filter(ModuleSpec.instantiate(filter_fcn_spec))
    dataset = dataset.traj_map(restructure, num_parallel_calls, deterministic=deterministic)
    dataset = dataset.traj_map(
        partial(
            normalize_action_and_proprio,
    ...
    *,
    ...
    with tf.device("/cpu:0"):
        ...
        for dataset_kwargs in dataset_kwargs_list:
            _, per_dataset_stats = make_dataset_from_rlds(
                **dataset_kwargs, train=train, save_statistics_dir=save_statistics_dir
            )
            dataset_sizes.append(per_dataset_stats["num_transitions"])
            all_dataset_statistics[dataset_kwargs["name"]] = per_dataset_stats
        ...
        for dataset_kwargs, threads, reads in zip(
            ...
            per_dataset_stats = all_dataset_statistics[dataset_kwargs["name"]]
            dataset, _ = make_dataset_from_rlds(
                **dataset_kwargs,
                train=train,
                num_parallel_calls=threads,
                num_parallel_reads=reads,
                dataset_statistics=dataset_statistics if dataset_statistics is not None else per_dataset_stats,
            )
        ...
        dataset: dl.DLataset = dl.DLataset.sample_from_datasets(datasets, sample_weights).shuffle(
        ...
        # save for later
        dataset.dataset_statistics = dataset_statistics if dataset_statistics is not None else all_dataset_statistics
