...
def generate_base_script(
    ...
    param_base_script = [
        ...
        (" --model.encoder_model_path ", path_model_learned),
    ]
...
def command_python(
    ...
    path_model_config = "./src/cond_fip/config/cond_fip_training.yaml"
    ...
    path_model_learned = get_path_pretrained_encoder(run_id, dir_model)
...
def main(
    ...
    run_name = "cond_fip_training_" + run_name
    command_base_script = command_python(dir_model, run_name, batch_size, factor_epoch, accumulate_grad_batches, run_id)
...
if __name__ == "__main__":
    ...
    # pretrained argument
    parser.add_argument("--run_id", type=str, help="run id of the pre-trained encoder", required=True)
