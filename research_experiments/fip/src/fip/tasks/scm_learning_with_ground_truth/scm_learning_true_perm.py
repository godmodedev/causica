...
class SCMLearningTruePerm(pl.LightningModule):
    """Lightning trainer for the task of causal discovery with true permutation."""
    ...
    def predict_graph(self, batch, threshold=0.1, mode="mean"):
        x, _, true_graph, *_ = batch

        true_sig = graph_to_perm_torch(true_graph).squeeze(0)
        x = x[:, true_sig]
    ...
    def test_step_cf(self, batch):
        ...
        mean_data = self.mean_data[:, true_sig]
        std_data = self.std_data[:, true_sig]
        f_data = f_data[:, true_sig]
        cf_data = cf_data[:, true_sig]

        int_index = torch.argmax((true_sig == int_index).float()).item()
