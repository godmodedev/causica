...
def create_heteroscedastic_rffgauss_sem(
    shapes: dict[str, torch.Size],
    random_features: torch.Tensor,
    coeff_matrix: torch.Tensor,
    graph: torch.Tensor,
    ...
    noise_dist = JointNoiseModule(independent_noise_modules)
    func = RFFFunctionalRelationships(shapes, random_features, coeff_matrix)
    log_func = HeteroscedasticRFFFunctionalRelationships(shapes, random_features, coeff_matrix, log_scale=True)
    return DistributionParametersSEM(graph=graph, noise_dist=noise_dist, func=func, log_func_rescale=log_func)
